...
class Problem():
    ...
    def ifThen_y_and_x_eq(self,number : 1|2|3|4, x1, x2, y1, y2):
        ...
        if(get_line_coeff((x1,y1),(x2,y2))) == 'vertical':
            ...
            self.problem += (self.vertices[number-1][0]==x1)
            self.problem += (y1-self.upperBound*(1-cy)<=self.vertices[number-1][1])
            self.problem += (self.vertices[number-1][1]<=self.upperBound*(1-cy)+y2)
        else:
...
def set_positionning_problem(rack1,rack2,room,point):
    ...
    # stick x1
    problem.ifThen_y_and_x_eq(1,min(x[2],x[0]),max(x[2],x[0]),min(y[2],y[0]),max(y[2],y[0]))
    problem.ifThen_y_and_x_eq(1,min(x[1],x[0]),max(x[1],x[0]),min(y[1],y[0]),max(y[1],y[0]))
    # stick x2
    problem.ifThen_y_and_x_eq(2,min(x[2],x[0]),max(x[2],x[0]),min(y[2],y[0]),max(y[2],y[0]))
    ...
    problem.ifThen_y_and_x_eq(3,min(x[0],x[2]),max(x[0],x[2]),min(y[0],y[2]),max(y[0],y[2]))
    # stick x4
    problem.ifThen_y_and_x_eq(4,min(x[1],x[3]),max(x[1],x[3]),min(y[1],y[3]),max(y[1],y[3]))
