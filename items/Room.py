...
class Room :    
    """This class allows to represent and manipulate a room"""

    def __init__(self, name : str, position : list, rotation, template : dict):
        self.name = name.replace(" ","")
        self.position = position
        self.rotation = rotation%360
        self.template = template
        self.axisOrientation = template.get("axisOrientation")
        self.size = template.get("sizeWDHm")
        self.floorUnit = template.get("floorUnit")

    def positionConform(self, testPosition : list) -> bool:
        """Verifies that the position is conform"""
        return len(testPosition) == 2 and isListOfNumbers(testPosition)
    
    def rotationConform(self, testRotation : float) -> bool:
        """Verifies that the rotation is conform"""
        return type(testRotation) in [float,int]
    
    def sizeConform(self, testSize : list) -> bool:
        """Verifies that the size is conform"""
    ...
    def templateConform(self, testTemplate : str) -> bool:
        """Verifies that the template is conform"""
        return type(testTemplate) == dict
    ...
    def setTemplate(self, newTemplate : dict) -> None:
    ...
    def isConform(self) : # TODO : complete template or not
        """Verifies that the attributes are conform"""
        boolean = True
