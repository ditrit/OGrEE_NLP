...
class Site :
    ...
    def getIndexBuilding(self, name : str):
        k = 0
        n = len(self.buildings) - 1
        if n < 0:
            raise IndexError("There is no building in this site.")
        while k < n or name != self.buildings[k].name:
            k += 1
        if k == n + 1:
    ...
    def getBuilding(self,name : str) -> Building:
        """Returns a Building instance from a Site thanks to its name. A ValueError is raised if there is no building with such name."""
        return self.buildings[self.getIndexBuilding(name)]
    
    def removeBuilding(self, name : str) -> None:
        """Removes a Building instance from a Site thanks to its name. A ValueError is raised if there is no building with such name.
